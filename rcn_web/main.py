            ...
            found_apps.append(
                {
                    ...
                    "status-code": app.get("status_code"),
                    "urls": [
                        {"url": app.get("scheme", "http") + "://" + app["site"] + i["path"], **i}
                        for i in get_storage_create("web-apps::app-links", parent_id=app['id']).get()
                    ],
                    "fuzzing-urls": [
                        {
                            "status": i["status"],
                            "url": app.get("scheme", "http") + "://" + app["site"] + i["path"],
                        }
                        for i in get_storage_create("web-apps::fuzzing-data", parent_id=app['id']).get()
                    ],
                    "js-links": [
                        {
                            **i,
                        ...
                        for i in get_storage_create("web-apps::js-links", parent_id=app['id']).get()
                    ],
                    "nuclei-scanning": get_storage_create("web-apps::nuclei-scanning", parent_id=app['id']).get(),
                    "js-secrets": get_storage_create("web-apps::js-secrets", parent_id=app['id']).get(),
                    "trufflehog-secrets": get_storage_create("web-apps::trufflehog-secrets", parent_id=app['id']).get(),
                }
            )
    ...
    for app in get_uniq_apps(target):
        ...
        try:
            ...
            for annotation in annotations:
                ...
                annotation['source_storage'] = "web-apps" # This maps to the 'web-apps' view
                all_annotations.append(annotation)
        except: pass
