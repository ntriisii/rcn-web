...
async def force_dump_data() -> JSONResponse:
    await get_target_storage().dump_data(force=True)
...
async def get_app_data(app_id):
    app = get_app_by_id(get_target_storage(), app_id)
    if not app:
...
async def get_app_more_data(req: Request):
    ...
    found_apps = []
    for i in ids:
        app = get_app_by_id(get_target_storage(), i)
        if not app:
            continue
...
async def get_data_view(entry, data_storage="app"):
    ...
    if data_storage == "app":
        app = get_app_by_id(get_target_storage(), entry)
        if not app:
            return JSONResponse({"error": "app not found"}, status_code=404)
        data = elisp_make_app_view_data(app)
...
async def get_entry_by_id(
    ...
    if not app_name:
        st = get_target_storage()[storage_name]
        if not st:
    ...
    app = None
    if app_name:
        app = get_app_by_id(get_target_storage(), app_name)
    if not app and app_name:
        app = get_app_by_site(get_target_storage(), app_name)
