...
async def mcp_interactive_ai_process_todo_notes(event, scheduled_md):
    ...
    async with get_unprocessed_annotations(
        "todo", scanner_name, event, match_storage_fn=web_match_storage
    ) as unscanned:
        if not unscanned:
            return
        todo_data = []
        for item in unscanned.values():
            ...
            if key.startswith("todo"):
                ...
                app_entry = next(
                    (a for a in todo_data if a["site"] == app["site"]), None
                )
                if not app_entry:
                    app_entry = {
                        "site": app["site"],
                ...
                app_entry["annotations"].append(
                    {"id": annotation_id, "key": key, "value": annotation_value}
                )
