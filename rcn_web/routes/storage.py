...
get_data_storages_matching = get_storage_data_matching


# Dummy storage manipulation functions
def add_to_data_storage(storage_index, data):
    return None


def del_storage_data(storage_index, ev):
    return None


def modify_storage_data(storage_index, ev, ex):
    return None
...
async def add_entry_annotation(req: Request):
    ...
    for ident in identifiers:
        for storage_name in storage_names or ["web-apps::annotations"]:
            if storage_name == "web-apps":
                ...
                if isinstance(entry_id, (int, str)):
                    app = get_app_by_site(target, entry_id)
                else:
                    app = target
                if app:
                    parent_id = (
                        ...
                        else getattr(app, "id", None)
                    )
                    nid = global_add_annotation(
                        entry_id=entry_id,
                ...
                if ident:
                    app = get_app_by_site(target, ident)
                else:
                    app = target
    ...
    return JSONResponse(
        {"count": len(added_annotations), "annotations": added_annotations}
    )
