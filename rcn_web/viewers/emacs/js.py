...
def elisp_view_js_flows(
    ...
    tabulated_entries, tabulated_format = elisp_make_js_flows_tabulated_entries(
        flow_storage, match_groups, *args, **kwargs
    )
...
def js_flows_preview_data(sto, page=0, reset_page_counter=False, match_groups=None):
    ...
    data_length = len(sto)
    tr["Found JS flows length "] = data_length
