...
class TestMCPStandardized:
    def test_view_apps_with_filter(self, client):
        ...
        with patch("rcn_web.core.utils.get_target_storage", return_value=mock_target):
            response = client.post(
                "/mcp/view",
                json={"collection": "web-apps", "filter": "entry['id'] == 1"},
            )
    ...
    def test_preview_flows(self, client):
        ...
        with patch(
            "rcn_web.core.utils.RemoteFlowsAdapter.get_instance",
            return_value=mock_storage,
        ):
            response = client.post("/mcp/preview", json={"collection": "flows"})
            assert response.status_code == 200
